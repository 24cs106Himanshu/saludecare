...
export default function SettingsPage() {
    const { user, login } = useAuth(); // login is typically used to set user context
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            // We need to update local storage user state. 
            // In many AuthContext setups, login(res.data) updates token & user.
            // Check the context to ensure how it updates or just temporarily cheat it by storing
            if (res.data.user) {
                // If the AuthContext expects a complete login object
                const token = localStorage.getItem('token');
                localStorage.setItem('user', JSON.stringify(res.data.user));
                // Reload the window to ensure everything picks up the new user data
